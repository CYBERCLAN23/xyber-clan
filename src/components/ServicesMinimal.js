                ...
                <div ref={headerRef} className={`mb-20 flex flex-col md:flex-row items-end justify-between gap-8 slide-up ${headerVisible ? 'visible' : ''}`}>
                    <div>
                        <span className="text-cyan-500 font-bold tracking-wider uppercase text-sm mb-4 block">
                            <EditableText contentKey={`${lang}.services.badge`} tag="span" fallback={t.services.badge} />
                        </span>
                        <h2 className="text-5xl md:text-7xl font-black tracking-tighter">
                            <EditableText contentKey={`${lang}.services.title`} tag="span" fallback={t.services.title} />
                        </h2>
                    </div>
                    <p className={`max-w-md text-lg ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                        <EditableText contentKey={`${lang}.services.subtitle`} tag="span" fallback={t.services.subtitle} />
                    </p>
                </div>
                            ...
                            <div className="flex flex-col md:grid md:grid-cols-12 gap-4 md:gap-8 items-start">
                                ...
                                <div className="md:col-span-5">
                                    <h3 className="text-2xl md:text-4xl font-bold group-hover:text-cyan-500 transition-colors duration-300 flex items-center gap-4">
                                        <EditableText contentKey={`${lang}.services.items.${index}.title`} tag="span" fallback={t.services.items[index].title} />
                                    </h3>
                                </div>

                                <div className="md:col-span-5 md:col-start-8">
                                    <p className={`text-base md:text-lg mb-4 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                                        <EditableText contentKey={`${lang}.services.items.${index}.desc`} tag="span" fallback={t.services.items[index].desc} />
                                    </p>
                                    <div className="flex flex-wrap gap-2">
                                        {service.tags.map((tag, tagIdx) => (
                                            <span key={tag} className={`text-xs px-2 py-1 rounded-full border ${isDark ? 'border-white/10 text-gray-400' : 'border-black/10 text-gray-600'}`}>
                                                <EditableText contentKey={`${lang}.services.items.${index}.tags.${tagIdx}`} tag="span" fallback={t.services.items[index].tags[tagIdx]} />
                                            </span>
                                        ))}
                                    </div>
                                </div>
